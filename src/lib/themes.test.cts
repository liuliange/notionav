...
test('注册麦金塔 1984 复古主题', () => {
  const theme = getAllThemes().find((item: { name: string }) => item.name === 'macintosh-1984');

  assert.ok(theme);
  assert.equal(theme.displayName, '麦金塔 1984');
  assert.equal(getThemeMode('macintosh-1984'), 'light');
});
